...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!localStorage.getItem("hasInitializedDefaults")) {
    ...
    // 设置默认弹幕API地址（使用新的弹幕API列表格式）
    // 只有当defaultApiUrl不为空时才添加默认弹幕源
    if (DANMAKU_CONFIG.defaultApiUrl) {
      const defaultDanmakuAPIs = [
        {
          name: "默认弹幕源",
          url: DANMAKU_CONFIG.defaultApiUrl,
        },
      ];
      localStorage.setItem("danmakuAPIs", JSON.stringify(defaultDanmakuAPIs));
      localStorage.setItem("selectedDanmakuApi", "0"); // 默认选中第一个
    } else {
      // defaultApiUrl为空，初始化为空数组
      localStorage.setItem("danmakuAPIs", JSON.stringify([]));
    }
...
async function showDetails(id, vod_name, sourceCode) {
  ...
  try {
    ...
    if (data.episodes && data.episodes.length > 0) {
      ...
      // 保存豆瓣ID（如果存在）
      currentDoubanId =
        data.videoInfo && data.videoInfo.douban_id
          ? data.videoInfo.douban_id
          : "";

      // 调试：输出 douban_id
      console.log(
        "Video Detail - douban_id:",
        currentDoubanId,
        "Full videoInfo:",
        data.videoInfo
      );
...
function playVideo(
  ...
  // 调试：输出所有参数
  console.log("playVideo called with:", {
    url,
    vod_name,
    sourceCode,
    episodeIndex,
  ...
  // 添加豆瓣ID参数（如果存在）
  if (doubanId) {
    watchUrl += `&douban_id=${encodeURIComponent(doubanId)}`;
    console.log("Adding douban_id to URL:", doubanId);
  } else {
    console.log("No douban_id to add");
  }
  ...
  try {
    ...
    // 保存豆瓣ID
    if (doubanId) {
      localStorage.setItem("currentDoubanId", doubanId);
    }
  } catch (e) {
  ...
  return episodes
    .map((episode, index) => {
      ...
      return `
        ...
        /"/g,
        "&quot;"
      )}', '${sourceCode}', ${realIndex}, '${vodId}', '${currentDoubanId}')"
...
// 渲染弹幕API列表
function renderDanmakuAPIsList() {
  const container = document.getElementById("danmakuApisList");
  if (!container) return;

  const danmakuAPIs = JSON.parse(localStorage.getItem("danmakuAPIs") || "[]");
  const selectedDanmakuApi = localStorage.getItem("selectedDanmakuApi") || "";

  if (danmakuAPIs.length === 0) {
    container.innerHTML =
      '<p class="text-xs text-gray-500 text-center my-2">未添加弹幕API</p>';
    return;
  }

  container.innerHTML = "";
  danmakuAPIs.forEach((api, index) => {
    const apiItem = document.createElement("div");
    apiItem.className =
      "flex items-center justify-between p-1 mb-1 bg-[#222] rounded";
    const isSelected = selectedDanmakuApi === index.toString();
    apiItem.innerHTML = `
            <div class="flex items-center flex-1 min-w-0">
                <input type="radio" name="danmakuApi" id="danmaku_api_${index}" 
                       class="form-radio h-3 w-3 text-blue-600 mr-1" 
                       ${isSelected ? "checked" : ""} 
            ...
            <div class="flex items-center">
                <button class="text-blue-500 hover:text-blue-700 text-xs px-1" onclick="editDanmakuApi(${index})">✎</button>
                <button class="text-red-500 hover:text-red-700 text-xs px-1" onclick="removeDanmakuApi(${index})">✕</button>
            </div>
        `;
    container.appendChild(apiItem);
...
// 选择弹幕API
function selectDanmakuApi(index) {
  const danmakuAPIs = JSON.parse(localStorage.getItem("danmakuAPIs") || "[]");
  if (index >= 0 && index < danmakuAPIs.length) {
    localStorage.setItem("selectedDanmakuApi", index.toString());
    // 同时更新旧的存储键，以兼容player.js
    localStorage.setItem(DANMAKU_CONFIG.storageKey, danmakuAPIs[index].url);
    showToast("已切换到弹幕源: " + danmakuAPIs[index].name, "success");
  }
}

// 显示添加弹幕API表单
function showAddDanmakuApiForm() {
  const form = document.getElementById("addDanmakuApiForm");
  if (form) {
    form.classList.remove("hidden");
  }
}
...
// 取消添加弹幕API
function cancelAddDanmakuApi() {
  const form = document.getElementById("addDanmakuApiForm");
  if (form) {
    form.classList.add("hidden");
    document.getElementById("danmakuApiName").value = "";
    document.getElementById("danmakuApiUrl").value = "";

    // 确保按钮是添加按钮
    restoreAddDanmakuApiButtons();
  }
}

// 添加弹幕API
function addDanmakuApi() {
  const nameInput = document.getElementById("danmakuApiName");
  const urlInput = document.getElementById("danmakuApiUrl");
  const name = nameInput.value.trim();
  let url = urlInput.value.trim();

  if (!name || !url) {
    showToast("请输入弹幕源名称和API地址", "warning");
    return;
  }

  if (!/^https?:\/\/.+/.test(url)) {
    showToast("弹幕API地址格式不正确，需以http://或https://开头", "warning");
    return;
  }
  ...
  const danmakuAPIs = JSON.parse(localStorage.getItem("danmakuAPIs") || "[]");
  danmakuAPIs.push({ name, url });
  localStorage.setItem("danmakuAPIs", JSON.stringify(danmakuAPIs));

  // 如果这是第一个弹幕API，自动选中它
  if (danmakuAPIs.length === 1) {
    selectDanmakuApi(0);
  }
  ...
  nameInput.value = "";
  urlInput.value = "";
  document.getElementById("addDanmakuApiForm").classList.add("hidden");
  showToast("已添加弹幕源: " + name, "success");
}

// 编辑弹幕API
function editDanmakuApi(index) {
  const danmakuAPIs = JSON.parse(localStorage.getItem("danmakuAPIs") || "[]");
  if (index < 0 || index >= danmakuAPIs.length) return;

  const api = danmakuAPIs[index];
  document.getElementById("danmakuApiName").value = api.name;
  document.getElementById("danmakuApiUrl").value = api.url;
  ...
  if (form) {
    ...
    const buttonContainer = form.querySelector("div:last-child");
    buttonContainer.innerHTML = `
            <button onclick="updateDanmakuApi(${index})" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded text-xs">更新</button>
            <button onclick="cancelEditDanmakuApi()" class="bg-[#444] hover:bg-[#555] text-white px-3 py-1 rounded text-xs">取消</button>
        `;
  }
}

// 更新弹幕API
function updateDanmakuApi(index) {
  const danmakuAPIs = JSON.parse(localStorage.getItem("danmakuAPIs") || "[]");
  if (index < 0 || index >= danmakuAPIs.length) return;
  ...
  danmakuAPIs[index] = { name, url };
  localStorage.setItem("danmakuAPIs", JSON.stringify(danmakuAPIs));

  // 如果更新的是当前选中的API，也要更新旧的存储键
  const selectedDanmakuApi = localStorage.getItem("selectedDanmakuApi");
  if (selectedDanmakuApi === index.toString()) {
    localStorage.setItem(DANMAKU_CONFIG.storageKey, url);
  }
  ...
  urlInput.value = "";
  document.getElementById("addDanmakuApiForm").classList.add("hidden");
  showToast("已更新弹幕源: " + name, "success");
}

// 取消编辑弹幕API
function cancelEditDanmakuApi() {
  document.getElementById("danmakuApiName").value = "";
  document.getElementById("danmakuApiUrl").value = "";
  document.getElementById("addDanmakuApiForm").classList.add("hidden");
  restoreAddDanmakuApiButtons();
...
// 恢复弹幕API添加按钮
function restoreAddDanmakuApiButtons() {
  const form = document.getElementById("addDanmakuApiForm");
  if (!form) return;
  const buttonContainer = form.querySelector("div:last-child");
  if (buttonContainer) {
    buttonContainer.innerHTML = `
            <button onclick="addDanmakuApi()" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded text-xs">添加</button>
            <button onclick="cancelAddDanmakuApi()" class="bg-[#444] hover:bg-[#555] text-white px-3 py-1 rounded text-xs">取消</button>
        `;
  }
}
...
function removeDanmakuApi(index) {
  ...
  const apiName = danmakuAPIs[index].name;
  const selectedDanmakuApi = localStorage.getItem("selectedDanmakuApi");

  // 从列表中移除API
  danmakuAPIs.splice(index, 1);
  localStorage.setItem("danmakuAPIs", JSON.stringify(danmakuAPIs));
  ...
  if (selectedDanmakuApi === index.toString()) {
    if (danmakuAPIs.length > 0) {
      ...
      // 没有API了，清空选择
      localStorage.removeItem("selectedDanmakuApi");
      localStorage.removeItem(DANMAKU_CONFIG.storageKey);
    }
  } else if (selectedDanmakuApi && parseInt(selectedDanmakuApi) > index) {
    // 如果删除的API在选中API之前，需要更新选中索引
    localStorage.setItem(
      "selectedDanmakuApi",
      (parseInt(selectedDanmakuApi) - 1).toString()
    );
  }

  renderDanmakuAPIsList();
  showToast("已移除弹幕源: " + apiName, "info");
}
