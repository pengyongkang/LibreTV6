...
(function () {
  ...
  window.fetch = async function (input, init) {
    let requestUrl;
    try {
      // 对于字符串URL，尝试直接解析（支持完整URL）
      // 如果是相对路径，则使用window.location.origin作为base
      if (typeof input === "string") {
        // 如果是相对路径（以/开头），使用当前origin
        if (input.startsWith("/")) {
